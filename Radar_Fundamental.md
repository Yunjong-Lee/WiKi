...
# 3.4. [푸리에 급수](https://ogu45.com/zbxe/school/74027)
  ...
  - 진폭 스펙트럼의 역할
    ...
    + 사각 펄스 신호는 푸리에 급수로 전개하면 무한개의 주파수 성분을 가진다. [그림. 주파수 합성에 의한 사각 펄스 신호의 근사화]은 스펙트럼에 존재하는 고조파들을 차례로 더하여 사각 펄스 신호를 합성하는 과정을 보인 것이다. 그림에서 보면 세 개의 고조파만 더해져도 사각 펄스와 비슷한 모양을 갖추지만, 아직 불연속점의 날카로운 모서리가 이 합성 신호에는 나타나지 않는다. 왜냐하면 날카로운 모서리를 구현하기 위해서는 급격히 변하는 (고주파수)성분이 필요하기 때문이다. 고조파의 수를 점진적으로 늘림에 따라 모서리는 서서히 날카로워지며 신호는 사각 펄스에 더욱 가까워진다.  
      ...
      > 고조파의 수를 늘려가도 불연속점 부근에서 오버슈트가 생기며 파형이 평탄하지 않고 진동하는 현상을 관찰할 수 있다. 이론적으로는 무한개의 고조파를 더하면 정확히 사각펄스가 되어야 하지만, 실제 고조파 합성 신호는 고조파의 수를 아무리 늘리더라도 불연속점 근처에서 항상 불연속 크기의 약 9%의 오버슈트가 생기며, 고조파수에 비례하여 진동도 더 빨라진다. 이러한 현상을 Gibbs 현상이라고 한다.  
      
  - 위상 스펙트럼의 역할
    + 각 주파수 성분의 시간축 상의 위치를 표시한다. 진폭 스펙트럼이 같더라도 위상 스펙트럼이 다르면 신호의 파형이 달라짐  
      
      ![기본파와 고조파의 위상이 ϕ1=ϕ3=0 인 신호와 ϕ1=ϕ3=−π2  로 합성된 신호](https://ogu45.com/zbxe/files/attach/images/96/027/074/8a9065f701a67c0c0bc43b473586bbb1.jpg)

    + 정현파의 위상 $\phi_1$과 $\phi_3$ 사이의 관계 : 시간축 상에서 똑같은 거리만큼 이동한다면, 신호의 파형은 바뀌지 않음, 이 경우, 제3고조파의 주파수가 기본파의 세배 이므로 같은 시간 이동에 상응하는 위상은 3배가 된다. 즉, $\phi_3 = 3\phi_1$으로 각 고조파가 그 주파수에 비례하는 위상각 만큼 달라지면 파형이 그대로 유지된다.
    + 불연속점과 같은 급격한 변화가 있는 신호(불연속점에서 순간적인 변화를 만들어내기 위해서는 스펙트럼에 포함된 모든 (또는 대부분의) 고조파 성분이 불연속점 직전에서 같은 부호를 가져야 하며, 불연속점 직후에서 반대 부호를 가져야 한다)의 경우, 불연속 적ㅁ에서 모든 고조파 성분이 동일한 부호 변화를 보이며 0점을 통과하도록 위상이 일치해야 한다.
    + 톱니파 신호의 파형과 함께 이 신호를 푸리에 급수로 전개한 결과의 첫 세 성분이 그려져 있다. 모든 주파수 성분의 위상은 불연속 신호를 푸리에 급수로 전개한 결과의 첫 세 성분이다. 모든 주파수 성분의 위상은 불연속 점인 t=±1 직전에는 모든 성분이 양이고 그 직후에는 음이다. 모든 고조파에서 일어나는 이러한 부호 변화가 더해져서 불연속점이 형성된다. t=0 에서 모든 고조파의 값은 0이 되지만 전후의 부호 변화가 일치하지는 않음을 볼 수 있다. 만약 위상 스펙트럼에 대한 조건이 맞지 않으면 모서리가 깎여나가고 퍼진 파형을 얻게 된다.  
      
      ![불연속위상스펙트럼의 역할](https://ogu45.com/zbxe/files/attach/images/96/027/074/382fc567405b834cf7b7fbb99b0781b4.jpg)  
   
    + 신호를 합성할 때 고조파의 진폭과 위상을 조합(푸리에 스펙트럼)해야 임의의 주기 신호 x(t)를 합성할 수 있음

# 3.5. [푸리에 변환](https://ogu45.com/zbxe/school/74027)
- 비주시 신호의 주파수 영역 변환의 도구
- 비주기 신호를 주기 T=∞ 인 주기 신호로 간주해 푸리에 급수를 확장

  ![주기신호의 크기변화에 따른 신호 변화](https://ogu45.com/zbxe/files/attach/images/96/027/074/dde7a5ca10668f19ae7b9e8bbf70a532.jpg)  
  ※ 주기 신호는 기본 주파수 $ω_0$의 정수배인 주파수 성분만 존재하기 때문에 $ω_0$의 간격으로 늘어선 선 스펙트럼으로 나타난다. 그런데 $ω_0$는 주기 T에 반비례하므로, 주기 T를 크게 할 수록 $ω_0$는 점점 작아져서 선 스펙트럼의 배열 간격이 점점 줄어들어 조밀해진다.  
  > 신호의 주기가 2배가 되면 스펙트럼의 배열 간격이 반으로 줄어들어 스펙트럼이 빽빽해진다. 주기를 크게 할 수록 스펙트럼은 더욱 조밀해짐. 주기가 무한대(T → ∞의 극한을 취하면)이면, $\omega_0 → 0$이 되어 스펙트럼이 연속적으로 이어지는 형태가 된다.  

$x(t) = \displaystyle\sum_{k = - \infty}^{ \infty} X_k e^{j\omega_0t} = \displaystyle\sum_{k = - \infty}^{ \infty} (\frac{\omega_0}{2\pi} \int_ \frac{-T}{2} ^\frac{T}{2} x(t)e^{-j\omega_0t} \mathrm{d}t)e^{-j\omega_0t} + \cdots , - {식 11} $  
T⟶∞ 의 극한을 취하면 ω0=2πT는 무한소가 되므로 ω0는 dω로, kω0는 연속 변수 ω로, 총합은 적분으로 바뀌므로  
$x(t) = \int_ {- \infty} ^{\infty} [\frac{d\omega}{2\pi} \int_ {- \infty} ^{\infty} e^{-j\omega t} \mathrm{d} t ]e^{j\omega t}$  
$ = \frac{1}{2\pi} \int_{- \infty} ^{\infty}[ \int_ {- \infty} ^{\infty} x(t)e^{-j\omega t} ]e^{j\omegat} \mathrm{d\omega} $  
