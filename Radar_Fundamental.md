...
# 3.4. [푸리에 급수](https://ogu45.com/zbxe/school/74027)
  정현파를 이용하여 신호의 표현을 찾는 문제 : 주기 신호를 주파수가 서로 다른 정현파의 합으로 나타낼 수 있음(푸리에, Fr) → 푸리에 해석이라고 한다.
  주기 신호는 그 신호와 같은 주기를 갖는 정현파와 이 정현파의 정수배의 주파수를 갖는 정현파들의 합으로 표현할 수 있다. 이 때 주기 신호와 주기(T)가 같은 정현파를 기본파(fundamental wave)라고 하고 이 정현파의 주파수 $f_0 = \frac{1}{T} \lbrace\omega_0 = \frac{2\pi}{T} \rbrace$를 기본 주파수(fundamental period)라 한다. 또, $kf_0(k\omega_0, k=2,3,4, ⋯ )$의 주파수를 갖는 정현파를 k고조파(harmonics)라고 한다.  
  ...
  $a_0 + \sum_{k=1}^{inf} a_k cosk\omega_0 t +$
